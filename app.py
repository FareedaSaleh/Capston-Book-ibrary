...
#Author routs
  ...
  def create_author(jwt):
    ...
    if new_auth_name is None or new_gender is None or new_count_book is None:
      abort(400)
    try:
      author=Author(auth_nam=new_auth_name,gender=new_gender,count_book=new_count_book)
      author.insert()
      return jsonify({
        "success":True,
        "author":author.id
 ...
 #search endponts
  ...
  def book_search():
    ...
    if search is None or search == '':
      abort(404)
    try:
      result=Book.query.order_by(Book.id).filter(Book.book_name.ilike('%{}%'.format(search))).all()
